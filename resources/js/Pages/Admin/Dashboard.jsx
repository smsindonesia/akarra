  ...
  return (
    <div>
      ...
      <div className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
        <CountCard label="Artikel" value={counts.articles} to={route('admin.articles.index')} />
        <CountCard
          label="Terbit"
          value={counts.published}
          to={route('admin.articles.index', { status: 'published' })}
        />
        <CountCard label="Draf" value={counts.drafts} to={route('admin.articles.index', { status: 'draft' })} />
        <CountCard label="Kategori" value={counts.categories} to={route('admin.categories.index')} />
        <CountCard label="Pesan Masuk" value={counts.contacts} to={route('admin.contacts.index')} />
        <CountCard
          label="Belum Dibaca"
          value={counts.unread_contacts}
          to={route('admin.contacts.index', { is_read: 0 })}
        />
      </div>
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-2">
        <section>
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link href={route('admin.articles.index')} className="text-[11px] uppercase tracking-[0.14em] text-gold hover:underline">
              Lihat semua
            </Link>
          </div>
            ...
            <ul className="divide-y divide-ink/10 border-y border-ink/10">
              {latestArticles.map((article) => (
                <li key={article.id} className="py-3">
                  <Link href={route('admin.articles.edit', article.slug)} className="text-[14px] text-ink hover:text-gold">
                    {article.title}
                  </Link>
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link href={route('admin.contacts.index')} className="text-[11px] uppercase tracking-[0.14em] text-gold hover:underline">
              Lihat semua
            </Link>
          </div>
            ...
            <ul className="divide-y divide-ink/10 border-y border-ink/10">
              {unreadContacts.map((contact) => (
                <li key={contact.id} className="py-3">
                  <Link href={route('admin.contacts.index')} className="text-[14px] text-ink hover:text-gold">
                    {contact.name}
                  </Link>
